...
extern bool solveSudokuBitMasks(vector<vector<int>>& board,
                                 vector<int>& rows,
                                 vector<int>& cols,
                                 vector<int>& grids);
...
void benchmarkBitMask(const string& filename, int numPuzzles)
{
    ...
    cout << "Starting benchmark with " << numPuzzles << " puzzles (BIT MASKING)...\n" << endl;
    ...
    for (int puzzleNum = 0; puzzleNum < numPuzzles; puzzleNum++) {
        ...
        bool success = solveSudokuBitMasks(board, rows, cols, grids);
    ...
    // Print statistics
    cout << "\n=== Benchmark Results (BIT MASKING) ===" << endl;
